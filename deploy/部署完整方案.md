...
# 创建项目目录
sudo mkdir -p /var/www/fangdu-system
sudo chown -R $USER:$USER /var/www/fangdu-system
...
| 入方向 | 允许 | TCP | 3006 | 0.0.0.0/0 | 后端API（或仅内网） |
| 入方向 | 允许 | TCP | 5432 | 内网IP | PostgreSQL（仅内网） |
...
# 应该返回 JSON 响应: {"status":"ok","message":"跟单系统API运行正常"}
```
...
# 更新代码
cd /var/www/fangdu-system && git pull && npm install && \
...
5. 运行健康检查: `./deploy/scripts/health-check.sh`
