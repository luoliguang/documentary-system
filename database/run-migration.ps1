...
# 使用方法: .\database\run-migration.ps1
...
$migrationPath = Join-Path $PSScriptRoot "migrations\001_add_production_manager_features.sql"
...
SELECT 
    ...
    ) THEN '✅ users.assigned_order_types' ELSE '❌ users.assigned_order_types' END as users_field,
    CASE WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'orders' AND column_name = 'order_type'
    ) THEN '✅ orders.order_type' ELSE '❌ orders.order_type' END as orders_field,
    CASE WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'delivery_reminders' AND column_name = 'is_admin_assigned'
    ) THEN '✅ delivery_reminders.is_admin_assigned' ELSE '❌ delivery_reminders.is_admin_assigned' END as reminders_field1,
    CASE WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'delivery_reminders' AND column_name = 'assigned_to'
    ) THEN '✅ delivery_reminders.assigned_to' ELSE '❌ delivery_reminders.assigned_to' END as reminders_field2;
"@
